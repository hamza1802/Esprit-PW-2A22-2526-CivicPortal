...
document.addEventListener('DOMContentLoaded', () => {
    // Vanilla JS Parallax & Kinetic interaction
    document.addEventListener('mousemove', (e) => {
        const brand = document.querySelector('.kinetic-text');
        if(!brand) return;
        const x = (e.clientX / window.innerWidth - 0.5) * 10;
        const y = (e.clientY / window.innerHeight - 0.5) * 10;
        brand.style.transform = `perspective(1000px) rotateX(${-y}deg) rotateY(${x}deg)`;
    });
