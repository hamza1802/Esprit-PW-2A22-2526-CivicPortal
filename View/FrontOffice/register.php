...
<section class="page-container" style="display: flex; justify-content: center; align-items: center; min-height: 100vh;">
    <div class="form-card" style="width: 100%; max-width: 600px; padding: 4rem 3rem;">
        <div style="text-align: center; margin-bottom: 2rem;">
            <img src="../assets/images/logo.png" alt="CivicPortal" style="height: 60px;" onerror="this.style.display='none'">
        </div>
        ...
        <form id="register-form" method="post" action="register.php" class="form-grid" novalidate>
            <input type="hidden" name="action" value="register">

            <div class="form-group full-width">
                <label for="name">Full Name</label>
                <input id="name" name="name" type="text" placeholder="YOUR NAME " value="<?= htmlspecialchars($old['name'] ?? '') ?>">
                <span class="inline-error" id="error-name" style="color: #ff4d4d; font-size: 0.8rem; margin-top: 0.5rem; display: block; font-weight: 700; text-transform: uppercase;"></span>
            </div>

            <div class="form-group full-width">
                <label for="email">Email</label>
                <input id="email" name="email" type="text" placeholder="YOUR@EMAIL.COM" value="<?= htmlspecialchars($old['email'] ?? '') ?>">
                <span class="inline-error" id="error-email" style="color: #ff4d4d; font-size: 0.8rem; margin-top: 0.5rem; display: block; font-weight: 700; text-transform: uppercase;"></span>
            </div>
            ...
            <div class="form-group full-width">
                ...
                <span class="inline-error" id="error-role" style="color: #ff4d4d; font-size: 0.8rem; margin-top: 0.5rem; display: block; font-weight: 700; text-transform: uppercase;"></span>
            </div>

            <div class="form-group">
                <label for="password">Password</label>
                <input id="password" name="password" type="password" placeholder="••••••••">
                <span class="inline-error" id="error-password" style="color: #ff4d4d; font-size: 0.8rem; margin-top: 0.5rem; display: block; font-weight: 700; text-transform: uppercase;"></span>
            </div>

            <div class="form-group">
                <label for="confirm_password">Confirm</label>
                <input id="confirm_password" name="confirm_password" type="password" placeholder="••••••••">
                <span class="inline-error" id="error-confirm_password" style="color: #ff4d4d; font-size: 0.8rem; margin-top: 0.5rem; display: block; font-weight: 700; text-transform: uppercase;"></span>
            </div>
            ...
            <div style="text-align: center; margin-top: 2rem; font-weight: 700; text-transform: uppercase; font-size: 0.9rem; grid-column: span 2;">
                Already have an account? <a href="login.php" style="color: var(--primary-navy); margin-left: 0.5rem;">Sign in</a>
            </div>
        </form>
    </div>
</section>
...
document.getElementById('register-form').addEventListener('submit', function(e) {
    ...
    fetch('../../Verification.php', {
        ...
        body: JSON.stringify({ action: 'register', data: data })
    })
    .then(res => res.json())
    .then(result => {
        if (result.success && !result.data?.errors) {
            ...
            if (Object.keys(errors).length > 0) {
                for (const field in errors) {
                    const errEl = document.getElementById(`error-${field}`);
                    if (errEl) errEl.textContent = errors[field].toUpperCase();
                }
            } else {
