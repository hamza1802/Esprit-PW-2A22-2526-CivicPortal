    ...
    <div class="form-card" style="max-width: 450px; padding: 3rem;">
        ...
        <?php if (empty($_GET['token'])): ?>
            ...
            <div style="text-align: center; margin-top: 2rem;">
                <a href="View/FrontOffice/login.php" class="btn btn-primary">Go to Login</a>
            </div>
        <?php else: ?>
            <form id="main-form">
...
document.getElementById('main-form')?.addEventListener('submit', async function(e) {
    ...
    try {
        const res = await fetch('Verification.php', {
            ...
            body: JSON.stringify({ action: 'reset_password', data: data })
        });
        const result = await res.json();
        
        if (result.success && !result.data?.errors) {
            window.location.href = 'View/FrontOffice/login.php';
        } else {
