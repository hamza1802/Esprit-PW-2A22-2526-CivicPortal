...
$success = isset($_GET['success']) ? 'If an account exists with that email, a password reset link has been sent.' : false;
$errors = [];
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <div class="form-card" style="max-width: 450px; padding: 3rem;">
        ...
        <form id="main-form">
            ...
            <div style="text-align: center; margin-top: 2rem;">
                <a href="View/FrontOffice/login.php" style="font-size: 0.85rem; color: #64748b; font-weight: 700; text-decoration: none; text-transform: uppercase;">Back to Login</a>
            </div>
        </form>
...
document.getElementById('main-form').addEventListener('submit', async function(e) {
    ...
    try {
        const res = await fetch('Verification.php', {
            ...
            body: JSON.stringify({ action: 'request_reset', data: data })
        });
        const result = await res.json();
        
        if (result.success && !result.data?.errors) {
            window.location.href = 'View/FrontOffice/forgot_password.php?success=1';
        } else {
