...
document.addEventListener('DOMContentLoaded', () => {
    ...
    //  ADD TRANSPORT TYPE
    // ─────────────────────────────────────────
    const addTypeForm = document.querySelector('form[action*="Verification.php"] input[name="action"][value="addTransportType"]');
    if (addTypeForm) {
        const form = addTypeForm.closest('form');
        const rules = [
            {
                field: '#name',
            ...
            {
                field: '#photo',
                validate: (el) => Validator.fileType(el, ['jpg', 'jpeg', 'png', 'gif', 'webp'], 'Photo')
            }
        ];

        Validator.attachLiveValidation(form, rules);
        form.addEventListener('submit', (e) => {
            if (!Validator.validateForm(form, rules)) {
                e.preventDefault();
            }
        });
    }
    ...
    // ─────────────────────────────────────────
    const updateTypeForm = document.querySelector('form[action*="Verification.php"] input[name="action"][value="updateTransportType"]');
    if (updateTypeForm) {
        const form = updateTypeForm.closest('form');
        const rules = [
            {
                field: '#name',
                validate: (el) => Validator.required(el.value, 'Type name') || Validator.minLength(el.value, 2, 'Type name') || Validator.maxLength(el.value, 50, 'Type name')
    ...
    // ─────────────────────────────────────────
    const addTransportForm = document.querySelector('form[action*="Verification.php"] input[name="action"][value="addTransport"]');
    if (addTransportForm) {
        const form = addTransportForm.closest('form');
        const rules = [
            {
                field: '#name',
            ...
            {
                field: '#idTransportType',
                validate: (el) => Validator.selectRequired(el.value, 'transport type')
            },
            {
                field: '#capacity',
    ...
    // ─────────────────────────────────────────
    const updateTransportForm = document.querySelector('form[action*="Verification.php"] input[name="action"][value="updateTransport"]');
    if (updateTransportForm) {
        const form = updateTransportForm.closest('form');
        const rules = [
            {
                field: '#name',
                validate: (el) => Validator.required(el.value, 'Vehicle name') || Validator.minLength(el.value, 2, 'Vehicle name')
    ...
    //  ADD TRAJET (Route)
    // ─────────────────────────────────────────
    const addTrajetForm = document.querySelector('form[action*="Verification.php"] input[name="action"][value="addTrajet"]');
    if (addTrajetForm) {
        const form = addTrajetForm.closest('form');
        const rules = [
            {
                field: '#departure',
                validate: (el) => Validator.required(el.value, 'Departure location')
            },
            {
                field: '#destination',
                validate: (el) => Validator.required(el.value, 'Destination location')
            },
            {
                field: '#idTransport',
            ...
            {
                field: '#departureTime',
                validate: (el) => Validator.required(el.value, 'Departure time') || Validator.futureDate(el.value, 'Departure time')
            },
            {
                field: '#price',
                validate: (el) => Validator.required(el.value, 'Price') || Validator.positiveNumber(el.value, 'Price')
            }
        ];
    ...
    //  UPDATE TRAJET (Route)
    // ─────────────────────────────────────────
    const updateTrajetForm = document.querySelector('form[action*="Verification.php"] input[name="action"][value="updateTrajet"]');
    if (updateTrajetForm) {
        const form = updateTrajetForm.closest('form');
        const rules = [
            {
                field: '#departure',
                validate: (el) => Validator.required(el.value, 'Departure location')
